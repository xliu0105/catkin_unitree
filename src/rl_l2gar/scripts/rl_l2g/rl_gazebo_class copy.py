  ...
  def sendCommand(self, action):
    ...
    if self.Ctrl_state == Control_State.PASSIVE:
      if self.args["REAL_ROBOT"]:
        for i in range(12):
          Cmd_real_robot.motorCmd[i].mode = 10
          Cmd_real_robot.motorCmd[i].q = 0.0
          Cmd_real_robot.motorCmd[i].dq = 0.0
          Cmd_real_robot.motorCmd[i].tau = 0.0
          Cmd_real_robot.motorCmd[i].Kp = 0.0
          Cmd_real_robot.motorCmd[i].Kd = 8.0
      else:
        for i in range(12):
          self.Cmd[i].mode = 10
          self.Cmd[i].q = 0.0
      ...
      if self.args["ctrl_mode"] == 0:  # position control，位置控制
        if self.args["REAL_ROBOT"]:
          for i in range(12):
            Cmd_real_robot.motorCmd[i].mode = 10
            Cmd_real_robot.motorCmd[i].q = action[i]
            Cmd_real_robot.motorCmd[i].dq = 0.0
            Cmd_real_robot.motorCmd[i].tau = 0.0
            Cmd_real_robot.motorCmd[i].Kp = self.args["ctrl_kp"]  # 这里用的PD值和isaac lab中的值需要是一样的
            Cmd_real_robot.motorCmd[i].Kd = self.args["ctrl_kd"]
        else:
          for i in range(12):
            self.Cmd[i].mode = 10
        ...
        if self.args["REAL_ROBOT"]:
          for i in range(12):
            Cmd_real_robot.motorCmd[i].mode = 10
            Cmd_real_robot.motorCmd[i].q = 0.0
            Cmd_real_robot.motorCmd[i].dq = action[i]
            Cmd_real_robot.motorCmd[i].tau = 0.0
            Cmd_real_robot.motorCmd[i].Kp = self.args["ctrl_kp"]  # 这里用的PD值和isaac lab中的值需要是一样的
            Cmd_real_robot.motorCmd[i].Kd = self.args["ctrl_kd"]
        else:
        ...
        if self.args["REAL_ROBOT"]:
          for i in range(12):
            ...
            Cmd_real_robot.motorCmd[i].q = 0.0
            Cmd_real_robot.motorCmd[i].dq = 0.0
            Cmd_real_robot.motorCmd[i].tau = action[i]
            Cmd_real_robot.motorCmd[i].Kp = 0.0
            Cmd_real_robot.motorCmd[i].Kd = 0.0
        else:
          for i in range(12):
            self.Cmd[i].mode = 10
